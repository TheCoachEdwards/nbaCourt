#' Plotting an NBA Court
#' @param none
#' @import ggplot2
#' @return returns ggplot object that can be used for NBA plots
#' @export
#' @examples
#' fullcourt() # plots court that you can layer various geoms onto
...
fullcourt <- function () {
  ...
  halfCircle <- circleFun(c(0, 5.25), 23.75*2, start=.12, end=.88, filled=FALSE)
  ggplot(data=data.frame(y=1,x=1),aes(x,y))+
    ...
    geom_path(data=data.frame(y=c(3,3,47,47),x=c(14,0,0,14)))+
    geom_path(data=halfCircle,aes(x=94-x,y=y+25))+
    geom_path(data=data.frame(y=c(3,3,47,47),x=c(80,94,94,80)))+
    coord_fixed()+
